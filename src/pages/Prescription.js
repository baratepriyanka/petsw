...
import { UserListHead, UserListToolbar, UserMoreMenu, PrescriptionToolbar } from '../sections/@dashboard/user';
// mock
...
const TABLE_HEAD = [
  ...
  // { id: 'patientimage', label: 'Patient Image', alignRight: false },
...
export default function Prescription() {
  ...
  const getPrescriptionData = async () => {
    ...
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/get-all-prescription/${loginId}`
    );
    const data = await response.json();
    if (response) {
      const prescriptionPatientData = data
        .sort((a, b) => {
      ...
      setPrescriptionData(prescriptionPatientData);
      setMedicinePageData([prescriptionPatientData]);
    }
  };
  ...
  const searchItems = (searchValue) => {
    ...
    if (searchInput !== '') {
      const filteredData = prescriptionData.filter(({ curElem, name }) => {
        return Object.values(curElem)
          .join('')
          .toLowerCase()
      ...
      // console.log("curElem");`
      setFilteredResults(prescriptionData);
    }
  };
  ...
  const deletePrescriptionRow = async (e) => {
    e.preventDefault();
    const aa = e.target.value;
    // console.log(aa);
    Axios.post(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/delete-prescription/${aa}`
    ) // <-- remove ;
      .then((res) => {
          ...
          <Container>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
              ...
              {/* <Button
                ...
                to="/dashboard/addPrescriptionJs"
                                  ...
                                  .map(({ curElem, name }) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        {patientSubStr.length < 8 ? (<TableCell align="center">{curElem.patient_name}</TableCell>) : <Tooltip title={curElem.patient_name} aria-label="patient_name">
                                        ...
                                        <TableCell style={{ width: 'maxWidth', display: 'flex',alignItems:"center" }}>
                                        <Box className='row-button6'>
                                         {curElem.added_from ==='0' ? (
                                            ...
                                            variant="contained"
                                            component={RouterLink}
                                            to={`/dashboard/InfoPrescriptionOpdList/${curElem.id}`}
                                          ...
                                          variant="contained"
                                          component={RouterLink}
                                          to={`/dashboard/InfoPrescriptionList/${curElem.id}`}
                                          sx={{ ml: 1, backgroundColor: '#2d2851' }}
