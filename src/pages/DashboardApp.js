...
export default function DashboardApp() {
  ...
  const getTotalOpdPatient = async () => {
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/total-opd-patient/${loginId}`
    );
  ...
  const getTotalIpdPatient = async () => {
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/total-ipd-patient/${loginId}`
    );
  ...
  const getTotalPatient = async () => {
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/total-patient/${loginId}`
    );
    setTotalPatient(await response.json());
    // console.log(opdData);
  };
  const [totalavabed, setTotalAvaBed] = useState([]);
  const getTotalAvaBed = async () => {
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/get-totalava-bednumber/${loginId}`
    );
    setTotalAvaBed(await response.json());
    // console.log(opdData);
  };
  ...
  const getTotalTodaysAppointment = async () => {
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/todays-appointment-count/${loginId}`
    );
        ...
        <Page title="Dashboard">
          <Container maxWidth="xl">
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <AppWidgetSummary
                  title="Total OPD Patient"
                  total={totalOpdPatient}
                  color="warning"
              ...
              <Grid item xs={12} sm={6} md={3}>
                <AppWidgetSummary
                  title="Total IPD Patient"
                  total={totalIpdPatient}
                  color="error"
                  icon={'eva:person-fill'}
                  onClick={handleClickIpd}
                />
              </Grid>
              <Grid item xs={12} sm={6} md={3}>
                <AppWidgetSummary
                  title="Available Cage / Kennel"
                  total={totalavabed}
                  color="error"
                  icon={'fa:bed'}
