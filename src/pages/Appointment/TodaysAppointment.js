...
export default function AllAppointment() {
  ...
  const loginId = loginData.hospital_id;
      ...
      const getTodaysAppointmentData = async () =>{
        // console.log("formErrors");
        const response = await fetch(`http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/get-all-todays-appointment/${loginId}`);
        const data =await response.json();
        if(response){
          const todayData = data.sort((a,b)=>{return a.name > b.name ? 1 : -1})
          .map((curElem, name) => {
            return {curElem, name};
           })
           setTodaysAppointmentData(todayData)
           setMedicinePageData([todayData])
          //  console.log(opdData)
        }
    }
    ...
    if (searchInput !== '') {
      const filteredData = todaysAppointmentData.filter(({ curElem, name }) => {
        console.log(curElem);
      ...
      // console.log("curElem");
      setFilteredResults(todaysAppointmentData);
    }
  };
    ...
    const deleteAppontmentRow = async (e) =>{
      ...
      console.log(aa);   
      Axios.post(`http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/delete-appointment/${aa}`) // <-- remove ;
      .then(res => {
      ...
      <Container>
        ...
        <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    <>
                    {/* // {
                    //       todaysAppointmentData.map(({curElem, name})=>{
                    //         console.log(curElem)
          ...
          <TablePagination
            ...
            component="div"
            count={todaysAppointmentData.length}
