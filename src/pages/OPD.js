...
export default function OPD() {
  ...
  const getOpdData = async () => {
    const response = await fetch(
      `http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/get-all-opdPatient/${loginId}`
    );
  ...
  const [deleteRow, setDeleteRow] = useState([opdData]);
  ...
  const deleteOpdRow = (e) => {
    ...
    axios
      .post(`http://${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/web/delete-opdpatient/${id}`) // <-- remove ;
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (select === '1') {
      ...
      const response1 = axios
        .post(urlReport, reportFile,{ headers: { 'Access-Control-Allow-Origin': '*' } })
        .then((res) => {
          const errMsg = '';
          setErrImg(errMsg);
          // console.log(res);
  ...
  const handleFileSubmit = (e) => {
    ...
    axios
      .post(urlfile, data,{ headers: { 'Access-Control-Allow-Origin': '*' } })
      .then((response) => {
        const errMsg = ' ';
        setErrImg(errMsg);
        setLoading(false);
  ...
  // const handleopenActive =({isSelected1}) =>{
  //   setUrl(window.location.href)
  //   console.log("ghfdhghfdgfdhg ")
  //   const isSelected = window.location.href;
  // }
  // console.log(url)
          ...
          <Container>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
              ...
              <Button
                ...
                to="/dashboard/registerNewPatient"
                              ...
                              ? // {
                                filteredResults
                                  ...
                                  .map(({ curElem, name }) => {
                                    ...
                                    const parentSubStr = parName.substr(0, 20);
                                    const paitName = curElem.patient_name;
                                    const patientSubStr = paitName.substr(0, 20);
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <TableCell align="center" key={curElem.id}>{curElem.patient_id} </TableCell>
                                        
                                        {patientSubStr.length < 15 ? (<TableCell align="center">{curElem.patient_name}</TableCell>) : <Tooltip title={curElem.patient_name} aria-label="patient_name">
                                        <TableCell align="center">{patientSubStr}...</TableCell>
                                        </Tooltip>}
                                       
                                        {parentSubStr.length < 15 ? (<TableCell align="center">{curElem.parent_name}</TableCell>)  :  <Tooltip title={curElem.parent_name} aria-label="parent_name">
                                        <TableCell align="center">{parentSubStr}...</TableCell>
                                        </Tooltip>}
                                        {/* <TableCell align="center">{curElem.prescriptionid}</TableCell> */}
                                        <TableCell align="center">{formatDate}</TableCell>
                                       

                                        {/* <TableCell align="center"> */}
                                          <TableCell style={{ width: 'maxWidth'}} >
                                            <Box style={{ display: 'flex'}} className='row-button7'>
                                              <Button
                                                size="small"
                                                type="button"
                                                variant="contained"
                                                className="edit-btn"
                                                component={RouterLink}
                                                to={`/dashboard/EditOpdList/${curElem.id}`}
                                              ...
                                              >
                                                <Box sx={style}>
                                                 ...
                                                 <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                                                    Are you sure you want to Delete OPD Patient?
                                                  </Typography>
                                                 </div>
                                                 <div className="text-center py-1"> 
                                                 <Button
                                                                ...
                                                                <>
                                                                  <div className="row">
                                                                    <div className="col-md-10 form-group ">
                                                                      ...
                                                                      { console.log(reportfile.file.length===0)} */}
                                                                       {reportfile.file.length ===0 ? ( <div style={{color:'red'}}>{errMsgvalidation}</div>) : ('')}
                                                                       {/* {reportfile.url === undefined ? ( <div style={{color:'red'}}>{errMsgvalidation}</div>) : ('')} */}
                                                                    </div>
                                                                  </div>
                                                                  <div className="text-center mt-3">
                                                                    <button
                                                ...
                                                <Box sx={style}>
                                                 ...
                                                 <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                                                  ...
                                                  <Button
                                                    ...
                                                    value={deleteId}
                                                    id=""
                                                    sx={{ mt: 2, ml: 1, backgroundColor: '#2768ff' }}
                                                  >
                                                    Ok
                                                  </Button>
